...
Provides manual push/pull synchronization of Obsidian vault settings (`.obsidian/` configuration files) between the client plugin and the server. Settings are stored separately from content files in a dedicated `settings` table. Unlike file sync, settings sync is manual-only (no real-time Socket.IO events).
...
Additionally, the client SHALL enforce safeguards on plugin files:

- Any path containing `node_modules` as a segment SHALL be excluded
- For files inside `plugins/<name>/`, only `main.js`, `manifest.json`, `styles.css`, and `data.json` are allowed — all other files are excluded
...
#### Scenario: Node modules excluded from push

- **WHEN** the client pushes settings and a plugin directory contains `node_modules/`
- **THEN** all files under `node_modules/` SHALL be excluded from the upload

#### Scenario: Only allowed plugin files synced

- **WHEN** the client pushes settings for a plugin `plugins/excalidraw/`
- **THEN** only `main.js`, `manifest.json`, `styles.css`, and `data.json` SHALL be uploaded — files like `README.md`, `package.json`, or nested source files SHALL be excluded
