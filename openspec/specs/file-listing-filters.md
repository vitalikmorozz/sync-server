...
Advanced filtering capabilities for the `GET /api/v1/files` endpoint, allowing clients to filter by file extension, content substring, partial path, and binary status. All filters are composable with each other and with existing pagination parameters.
...
### Search file content by substring
...
### All filters composable with pagination
