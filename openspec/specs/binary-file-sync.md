...
Binary files (images, PDFs, archives, etc.) are synchronized between clients and server using base64 encoding. The server determines binary status from the file extension, and clients encode/decode binary content as base64 strings for transport and storage.
...
### Client-side base64 encoding for binary files
...
### Hash computation on stored representation
...
### Binary file conflict resolution uses server-wins
...
### Binary files included in all sync phases
...
### Content size limit applies to encoded content
